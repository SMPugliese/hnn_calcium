...
class L5Pyr(Pyr):
    ...
    def create_all_IClamp(self, p):
        ...
        for sect_name in sect_list_IClamp:
            ...
            # sect_name_dict = {'soma': 'soma', 'apical_2': 'apical_tuft'}
    ...
    def __lin_dist (self, x, gsoma, gdend, gdist):
        # Returns g(x):
        # g(x) =  gdend, for x >= gdend
        #      =  (gdend-gsoma)/gdist * x + gsoma, for 0 < x < gdend [line between points (0, gsoma) and (gdist, gdend)]
        g = gsoma + x * (gdend - gsoma)/gdist
        if gdend < gsoma and g < gdend: return gdend
        elif gdend > gsoma and g > gdend: return gdend
        else: return g

    def __insert_distributed_channels (self, seg):
        # Inserts dendritic channels according to the following distributions:
        #     Kv channels are exponentially distributed according to [dend density] + [soma density]*e^(-0.006*[distance])
        #     Na and Ca channels are distributed as a linear ramp from the soma density to the dend density, where the
        #         line plateaus off at some value, right now just hard coded as 962 and 1501 respectively (this shape: _/¯ or ¯\_)
    ...
    def __synapse_create(self, p_syn):
        ...
        # self.apicaltuft_gabaa = self.syn_create(self.dends['apical_2'](0.5), p_syn['gabab'])
